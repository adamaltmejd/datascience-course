...
This guide provides step-by-step instructions on how to install R (with RStudio), Python, Julia, and Stata. We will primarily use R in class, and you can complete this course without installing any of the other interpteters, but I encourage you to explore different languages throughout the class. This document also covers setting up the version control system Git and creating an account on GitHub.com, installing my favorite code editor (Visual Studio Code), configuring SSH keys for GitHub, and creating your first GitHub repository.
...
To launch a terminal start the Git Bash application on Windows, on Mac, you can find the built-in Terminal app under `Applications/Utilities/Terminal`. The terminal window should look something like this (on Mac, if you haven't changed anything):
...
Some R packages require X11 for creating graphics. You can download and install it [here](https://www.xquartz.org/index.html), or install using brew:
...
Next, we are going to install an integrated development environment (IDE) for writing code. IDEs are enhanced text editors with many extra features. You may have tried the Stata do-file editor or RStudio before. I prefer VS Code and encourage you to try it. In class, we'll see how to use VS Code to connect to servers, access the terminal, create plots, browse data, and more. You can do powerful things like find and replace across multiple files or edit several rows of text at once. Best of all you can access the GitHub Copilot AI assistant for free, right inside VS Code. And if you register for Github Education you even get access to the pro version free of charge. Under additional software you will find instructions for installing alternative IDEs.

::: {.panel-tabset}

### Windows

Download and install VS Code from [VS Code's download page](https://code.visualstudio.com/download). Keep the default options throughout the install, but I suggest also select to configure the "Open with Code" context menus. That way you can right click on a folder to open it directly in VS Code.

By default, VS Code will use Powershell for the integrated terminal. You probably wan to change this to Git Bash. Do so by pressing Ctrl+Shift+P and start writing *Select default profile*, press enter and use the arrow keys to chose *Git Bash*.
...
To get VS Code working more like RStudio you need the [VS Code R extension](https://marketplace.visualstudio.com/items?itemName=REditorSupport.r). With VS Code open go to **Extensions** in the sidebar, either by clicking, using the shortcut Ctrl/Cmd+Shift+X, or from the universal shortcut Ctrl/Cmd+Shift+P and then start writing *install extensions*. At the top of the extension pane, enter *R* in the *Search Extensions in Marketplace* input box then click *Install extension*. You can also install it from [here](https://marketplace.visualstudio.com/items?itemName=REditorSupport.r).

![&nbsp;](Rextension.png){width=600px}

After having installed the extension, you will also need to install two R packages to get everything working. Easiest way to do so is to open an R terminal in VS Code using the shortcut Ctrl/Cmd+Shift+P and picking *Create R Terminal*. In the R terminal window write the command below and press enter. This should install the `languageserver`, which is used by R to talk to VS Code and provide e.g., help about functions. `httpgd` is used to show the plots you make inside VS Code.

Run this from an **R terminal** to install packages:
...
Here is a [nice introduction](https://code.visualstudio.com/docs/languages/r) to using R in VS Code.

### VS Code R debugger (optional)

You can also get the great debugging features that are available in R Studio into VS Code using the [VS Code R debugger extension](https://marketplace.visualstudio.com/items?itemName=RDebugger.r-debugger). Similarly to the VS Code R extension, it relies on an R package ([documentation](https://manuelhentschel.github.io/vscDebugger/)) that you will also need to install.

```r
# Run from an R terminal
install.packages("vscDebugger")
```

The extension is still in active development and in my experience has been quite buggy. But it might already be working much better since when I last used it!
...
### Registering for GitHub and GitHub Education

Next, go to [GitHub.com](https://github.com/) to register an account (if you don't already have one). Use the same email you set in your Git config. After registering and verifying your account, visit [https://github.com/education/students](https://github.com/education/students) to sign up for GitHub Education. You will need to enter an SU email address (and link it to your account). This should be enough to get verified. Once you are registered and verified, try the Copilot Chat in VS Code again. You should have access to more advanced models like Claude 3.7.
...
You now have everything you need to connect to servers and use GitHub over SSH. You wil however have to enter the passphrase each time you use your key. To save the passphrase on your computer, so that you only need your computer password you can use SSH Agent. Read more in the optional section.
...
That is what you see if everything works. If not, try to read the error message and debug. The [official documentation](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account) could be helpful, or try Google. If it works, you can now clone repositories using SSH rather than https. Try it out on the course repository! We will use SSH to connect to servers in the course.

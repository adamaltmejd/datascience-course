...
Or let a condition evalute to a logical vector on the fly:
...
::: notes
A `data.frame` is built by combining equal-length vectors into one tabular object. While `list()` would allow you to supply different length vectors, `data.frame()` would instead recycle the shorter ones.
:::
...
```{r}
#| output-location: default
recent_panel <- subset(panel_df, year >= 2020)
boxplot(

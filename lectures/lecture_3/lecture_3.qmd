...
::: aside
You can also use `=` for assignment in R. But `<-` is by far the most common, and recommended by the [Tidyverse style guide](https://style.tidyverse.org/). The main reason is that `=` has a special use for arguments in function calls (later), so using `<-` for assignment helps avoid confusion. However, `<-` for assignment can also cause confusion like between `a < -5` (comparison to `-5`) and `a <- 5` (assignment).
:::
...
::: notes
Assignment is different from comparison although the symbols look similar. `=` assigns an object to a name, `==` compare two objects. Logical operators like `&` and `|` combine logical values, not numbers. The result of a logical expression is always `TRUE` or `FALSE`.
:::

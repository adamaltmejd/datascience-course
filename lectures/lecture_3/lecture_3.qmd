...
::: notes
R understands the difference between `c` the function and `c` the variable here because of the parentheses. However, this is still very sloppy coding because it creates confusion for the reader and can lead to errors in more complex cases. Try printing `c` after the assignment, it will not refer to the function anymore. If you use Positron or enable diagnostics in VSCode-R you should get warnings if you try to assign to a name that is already in use by a built-in function.
:::
...
# Objects And Types {background-color="#40666e"}
...
# Logic And Missingness {background-color="#40666e"}
...
# Control Flow {background-color="#40666e"}
...
# Functions {background-color="#40666e"}
...
- Convenient but bug prone: avoid hidden dependencies

::: notes
This is scoping in practice. R first looks for a name inside the function call itself, then outward to surrounding environments. That is useful, but it also creates a common failure mode: code silently depends on an outer object that was never passed as an argument, or errors because the author assumed a name would already exist.
:::

## `lm()` cannot find names inside a table
...
::: notes
A common error, especially when coming from Stata. The formula uses the column names, but those names live inside the table rather than in the global environment. Passing `data = toy_df` tells the function to evaluate the formula inside that data frame.
:::
...
## Next lecture: Vectors, Tables, and Tidy Thinking {.title-slide background-color="#40666e"}

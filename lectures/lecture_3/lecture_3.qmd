...
- Much of what you will learn applies to all programming
...
- Much more dependent on external packages
  - Huge catalogue of user-created packages
  - The problem with open source: some packages are outdated or abandoned
  - You will need to learn to search for, install and load packages

## Coding environment recap

- Make sure you have `R` and `VS Code` installed
- You will also need the `VS Code R extension`
- Open a VS Code R terminal now
- Let's make sure it works for all of you before we continue
...
Factors are a special type that stores character vectors as integers. Each factor level is linked to a label. Good for memory efficiency and readability. Similar to how value labels work in Stata.
...
Raw data often comes encoded. For example, SCB delivers the variable `Kon` (sex) which can take values `1` and `2`

```{r}
kon <- c(1,2)
```

We can use factors to label the values to make it easier to use:

```{r}
factor(kon, levels = c(1, 2), labels = c("Man", "Woman"))
```
...
# Iteration {background-color="#40666e"}

## Loops

We often want to repeat the same operation many times.

Loops are good when each iteration is somehow dependent on the results of the previous.

```{r}
v <- c("foo", "bar", "foofoo", "barbar")
```
...
Before you write a loop, ask: Do I really need to iterate? Vectorized operations are often **much faster**.

You might have noticed we can do things like:

```{r}
1 + c(3,4)
```

This command runs `` `+`(1, 3) `` and `` `+`(1, 4) ``, i.e. it operates on both elements of the vector.

## Vectorized operations (cont.)

Similarly, we can *apply* any arbitrary function to each element:

```{r}
#| output-location: default
sapply(c(1,2,3), FUN = function(x) paste0(x, "^2=", x^2))
```

There are three versions:

- `apply` for matrices
- `lapply` for vectors and lists
- `sapply` which is just `lapply(..., simplify = TRUE)`

::: {.aside}
The `parallel` package provides corresponding multi-core operations.
:::

...
<div style='position: relative; padding-bottom: 56.25%; padding-top: 35px; height: 0; overflow: hidden;'><iframe sandbox='allow-scripts allow-same-origin allow-presentation' allowfullscreen='true' allowtransparency='true' frameborder='0' height='315' src='https://www.mentimeter.com/app/presentation/al2hjcty4jhgmqz262uiauagdgy2bagc/embed' style='position: absolute; top: 0; left: 0; width: 100%; height: 100%;' width='420'></iframe></div>
...
- Will make you more productive, and more *employable*
...
- Code is reusable and replicable
- "Emulated" by Stata `.do`-files:
  - Collections of commands
  - Different from programming: less flexible, lacks many features
- Still, some things are better done in Excel (or Stata) --- always ask yourself what the best and simplest tool for the job is!
...
![&nbsp;](tidy-data.png){fig-align="center" width=100%}
...
- Example from my work: collapsing education achievement data
...
- Instead, use functions, loops, or vectorized calls
- A smaller codebase is easier to understand and debug

## Be consistent

- Use the same coding style and naming convention
- For example: don't mix `snakeCase` variable names with `underscore_based` names,
- Use the same indentation (2 or 4 spaces)

## Document everything

- Always include a README-file that explains your project
- Use comments to document what your code is doing
- Start functions with a comment explaining the function and its arguments

## Be verbose

- By writing verbose code, you will not need as many comments:
- Call a variable `high_scool_gpa`, not `hsgpa`, (or worse `question12` 😱)
- Same applies to data labels:
  - `edu_level = primary, secondary, high school` not `1,2,3`
  - `female = TRUE,FALSE` not `Sex = 1,2`^[SCB actually does this still!]
...
- See Problem set 0 for instructions!

# Let's do some data science! {background-color="#40666e"}

## Example 1: Gapminder data

Let's load and summarize the famous Gapminder data in R, provided by the `gapminder` R package.

## Example 2: LLM data processing

Let's build a script that calls the OpenAI API to process some survey feedback I just collected.
...
*Problem set 0*
- Setting up data science computer environment

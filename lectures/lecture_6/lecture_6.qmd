...
::: aside
It is often worthwhile to spend some time on the import step to make sure wranling starts from a data set that is as clean and usable as possible. Import tools are often better at dealing with raw file issues than later code.
:::
...
```{r}
dogs[, mean(price), by = .(insured, white = color == "White")]
```

...
## A quick plot catches what tabular checks miss
...
::: notes
Tabular validation catches duplicates and `NA`s but tends to miss visible patterns: a week of zeros, a sudden 10× jump, a gap that lines up with a date-format change. A single one-line plot of counts over time or by group is usually enough as a first check after a join or reshape.
:::
...
dogs[breed == "[Cc]orgi", breed := "Pembroke Welsh Corgi"]

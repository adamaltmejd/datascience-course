...
Base R has `reshape()` but it is clunky. `data.table` has two more intuitive functions:
...
## When cell are not unique, `dcast` aggregates
...
small_events <- events[
  ...
  _[!(dog_id == "3382" & event_type == "vet_check")]
...
- Keep validating: row counts, unit of observation, and unmatched rows are cheap to re-check after every join, reshape, or type change

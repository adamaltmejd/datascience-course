...
`str()` is the probably the most useful inspection command in R
...
::: {.aside}
In RStudio, go to Tools → Global Options → General, and set "Save workspace to .RData on exit" to "Never". Positron already defaults to off. To disable globally (including in VSCode), you could replace `q()` in your `.Rprofile` with:
```r
q <- function(save = "no", ...) quit(save = save, ...)
```
:::

## Example 1: Selecting a `df` column
...
## Example 1: Selecting a `df` column (cont.)
...
## Enter Browser on function call: `debug()`

- `debug(<function_name>)` browses function when its called
- `debugonce(<function_name>)` enters `browser()` only on the next `<function_name>` call
- Use `undebug(<function_name>)` to turn off debugging
...
- VSCode, Positron, and RStudio all have built-in debuggers
...
Do you know what the code is supposed to do? Why not verify that is what it is actually doing? 
...
- Catches things errors and tests won't:
  - `any(is.na(x))` → suggests `anyNA(x)`
...
- Start writing code and `Copilot` will start suggesting
- `Tab` to accept, or keep writing to get a different one
- Suggestions are based on file context, not just the current line
- Write comments to guide the suggestions:
...
- Highlight code and press `💡` to ask Copilot to **modify** or **review**
- On R console errors, Positron assistant can help:
...
```text
In 01_traceback_test.R, explain why
check_reference_row() fails and propose
the smallest fix. Don't rewrite the
script or add packages.
```
:::
::::
...
Agents are LLM text completion engines. Their reasoning process is only as good as the input you give them.
